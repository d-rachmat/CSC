...
public class beritatwit : MonoBehaviour
{
    ...
    void updateData()
    {
        for (int i = 0; i < tweets.Length; i++)
        {
			...
			twitContainer.transform.GetChild(1).GetComponent<Text>().text = newUser.name;
			twitContainer.transform.GetChild(2).GetComponent<Text>().text = tweets[i].text;
			twitContainer.transform.GetChild(3).GetComponent<Text>().text = tweets[i].FormatedDateTime.Day +" "+tweets[i].FormatedDateTime.Month;
    ...
    IEnumerator GetIcon()
    {
        ...
        using (WWW www = new WWW(newUser.profile_image_url))
        {
			...
			GameObject[] GO = GameObject.FindGameObjectsWithTag("newsContainer");
			for (int i = 0; i < GO.Length; i++) {
				GO[i].transform.GetChild(0).GetComponent<Image>().sprite = Sprite.Create(tex, new Rect(0, 0, tex.width, tex.height), new Vector2(0.5f, 0.5f), 100.0f);
				GO [i].transform.localScale = new Vector3 (1f, 1f, 0f);
			}
        }
    }
